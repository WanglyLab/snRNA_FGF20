...
dat1 <- Read10X('./TAC_1/output/filter_matrix/', gene.column = 1)
dat2 <- Read10X('./TAC_2/output/filter_matrix/', gene.column = 1)
dat3 <- Read10X('./TAC_3/output/filter_matrix/', gene.column = 1)

dat4 <- Read10X('./Sham_1/output/filter_matrix/', gene.column = 1)
dat5 <- Read10X('./Sham_2/output/filter_matrix/', gene.column = 1)
dat6 <- Read10X('./Sham_3/output/filter_matrix/', gene.column = 1)

dat7 <- Read10X('./TAC+SLNP@Fgf20_1/output/filter_matrix/', gene.column = 1)
dat8 <- Read10X('./TAC+SLNP@Fgf20_2/output/filter_matrix/', gene.column = 1)
dat9 <- Read10X('./TAC+SLNP@Fgf20_3/output/filter_matrix/', gene.column = 1)
...
colnames(dat7) <- paste0('Fgf20_1_', colnames(dat7))
colnames(dat8) <- paste0('Fgf20_2_', colnames(dat8))
colnames(dat9) <- paste0('Fgf20_3_', colnames(dat9))
...
meta_dat <- data.frame(
  sample = colnames(combined_dat),
  Group = c(rep("TAC", ncol(dat1)+ncol(dat2)+ncol(dat3)),
            rep("Sham", ncol(dat4)+ncol(dat5)+ncol(dat6)),
            rep("TAC+SLNP@Fgf20", ncol(dat7)+ncol(dat8)+ncol(dat9)))
)
rownames(meta_dat) <- meta_dat$sample
